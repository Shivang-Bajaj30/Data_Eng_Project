...
public class NoteService {
    ...
    public UploadResult upload(User moderator, String title, String description, String subject,
                               String classId, String[] tags, String fileName) {
        ...
        note.fileName = fileName == null || fileName.isBlank()
                ...
                : fileName;
        note.fileUrl = "https://files.notevault.demo/" + note.id + "/" + note.fileName;
        note.fileHash = "sha256-" + note.id; // real SHA-256 when R2 upload is wired
        ...
        if (!notes.findByFileHashAndIsDeletedFalse(note.fileHash).isEmpty()) {
